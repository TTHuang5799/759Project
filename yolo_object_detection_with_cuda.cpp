...
#else
// CPU-based frame processing function as an alternative
void process_frame_cuda(const cv::Mat& input_frame, cv::Mat& output_frame)
{
    // Ensure the input image has 3 channels (BGR format)
    CV_Assert(input_frame.type() == CV_8UC3);

    int width = input_frame.cols;
    int height = input_frame.rows;
    int channels = input_frame.channels();

    // Initialize output_frame with the same size as input_frame but with a single channel
    output_frame.create(height, width, CV_8UC1);

    // Process each pixel
    #pragma omp parallel for
    for (int y = 0; y < height; ++y)
    {
        const uchar* input_row = input_frame.ptr<uchar>(y);
        uchar* output_row = output_frame.ptr<uchar>(y);

        for (int x = 0; x < width; ++x)
        {
            int idx = x * channels;
            uchar b = input_row[idx];
            uchar g = input_row[idx + 1];
            uchar r = input_row[idx + 2];
...
void process_video(const std::string& video_path, bool is_cuda, const std::vector<std::string>& class_list, SafeQueue<std::pair<std::string, cv::Mat>>& display_queue)
{
    ...
    for (int i = 0; i < total_frames; ++i)
    {
        if (!frames[i].empty())
        {
            ...
            // Detection step on the original color frame
            detect(frames[i], net, class_list);

            // Optional: Process frame for display purposes (e.g., convert to grayscale)
...
#else
        std::cerr << "CUDA is not supported on this system. Falling back to CPU.\n";
        is_cuda = false;
#endif
    }
